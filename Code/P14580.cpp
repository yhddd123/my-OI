// Problem: P14580 【模板】有源汇上下界最小流
// Contest: Luogu
// URL: https://www.luogu.com.cn/problem/P14580
// Memory Limit: 512 MB
// Time Limit: 1000 ms
// Written by yhm.
// Start codeing:2026-02-23 22:10:51
// 
...
bool bfs(){
	queue<int> q;
	for(int i=1;i<=n+2;i++)dis[i]=0,rad[i]=head[i];
	dis[s]=1,q.push(s);
	while(!q.empty()){
		int u=q.front();q.pop();
...
void work(){
	n=read();m=read();S=read(),T=read();s=n+1,t=n+2;
	for(int i=1;i<=m;i++){
	...
	if(flow!=sum){puts("N");return ;}
	flow=e[2*(m+1)+1].w;e[2*(m+1)].w=e[2*(m+1)+1].w=0;
	s=T,t=S;
	while(bfs())flow-=dfs(s,inf);
	printf("%d\n",flow);
}
