...
// Time Limit: 1000 ms
// Written by yhm.
// Start codeing:2024-07-21 16:15:20
// 
...
void work(){
	n=read();m=read();
	int mx=0;for(int i=1;i<=n;i++)a[i]=read(),mx=max(mx,a[i]);
	for(int i=1;i<=n;i++){
		for(int j=i;j<=n;j++){
			id[i][j]=++idx;
		}
	}
	s=idx+mx+1,t=idx+mx+2;
	for(int i=1;i<=n;i++){
		for(int j=i;j<=n;j++){
			int val=read();
			if(i==j){
				if(m)add(id[i][j],idx+a[i],inf);
				val-=a[i];	
			}
			else{
			...
			if(val<0)add(id[i][j],t,-val);
			else add(s,id[i][j],val),ans+=val;
		}
	}
	if(m)for(int i=1;i<=mx;i++)add(idx+i,t,i*i);
