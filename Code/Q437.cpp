...
vector<int> id[maxn];
int dep[maxn];
void dfsid(int u,int fa,int tp){
	id[tp].pb(u);dep[u]=dep[fa]+1;
	for(int v:e[u])if(v!=fa)dfsid(v,u,tp);
}
vector<int> createFunTour(int N, int Q) {
	n=N;
	siz[0]=n;for(int i=1;i<n;i++)siz[i]=attractionsBehind(0,i);
	for(int i=1;i<n;i++)if(n-siz[i]<=n/2&&siz[i]<siz[rt])rt=i;
	for(int i=0;i<n;i++)if(i!=rt)dep[i]=hoursRequired(rt,i);
	int v1=0,v2=0,v3=0;
	...
	// cout<<v1<<" "<<v2<<" "<<v3<<"\n";
	if(siz[v1]>n/2)siz[v1]=n-siz[rt];
	if(siz[v2]>n/2)siz[v2]=n-siz[rt];
	if(siz[v3]>n/2)siz[v3]=n-siz[rt];
	for(int i=0;i<n;i++)if(i!=rt){
		if(attractionsBehind(i,v1)!=siz[v1])id[v1].pb(i);
		else if(attractionsBehind(i,v2)!=siz[v2])id[v2].pb(i);
		else id[v3].pb(i);
	}
	vector<int> ans;
	sort(id[v1].begin(),id[v1].end(),[&](int u,int v){return dep[u]<dep[v];});
	sort(id[v2].begin(),id[v2].end(),[&](int u,int v){return dep[u]<dep[v];});
	sort(id[v3].begin(),id[v3].end(),[&](int u,int v){return dep[u]<dep[v];});
	if(id[v1].size()<id[v2].size())swap(v1,v2);
	if(id[v1].size()<id[v3].size())swap(v1,v3);
	// cout<<id[v1].size()<<" "<<id[v2].size()<<" "<<id[v3].size()<<"\n";
	while(id[v1].size()){
		// cout<<id[v1].size()<<" "<<id[v2].size()<<" "<<id[v3].size()<<"\n";
		if(id[v1].size()==id[v2].size()+id[v3].size()+1)break;
		ans.pb(id[v1].back()),id[v1].pop_back();
		if(id[v2].size()&&id[v3].size()&&(dep[id[v2].back()]<dep[id[v3].back()]||(dep[id[v2].back()]==dep[id[v3].back()]&&id[v2].size()<id[v3].size())))swap(v2,v3);
		if(!id[v2].size())swap(v2,v3);
		swap(v1,v2);
	}
	for(int val:id[v3])id[v2].pb(val);
	while(id[v1].size()){
		ans.pb(id[v1].back()),id[v1].pop_back();
		swap(v1,v2);
	}
	ans.pb(rt);
	// cout<<ans.size()<<" "<<id[v1].size()<<" "<<id[v2].size()<<" "<<id[v3].size()<<"\n";
	return ans;
}
