...
void work(){
	...
	for(int s=0;s<(1<<n);s++){
		for(int t=0;t<(1<<m);t++)if(g[s][t]){
			...
			// cout<<s<<" "<<t<<" "<<g[s][t]*16%mod<<" "<<no[s][(1<<m)-1-t]*16%mod<<" "<<f[(1<<n)-1-s][(1<<m)-1-t]*16%mod<<" "<<g[s][t]*no[s][(1<<m)-1-t]%mod*f[(1<<n)-1-s][(1<<m)-1-t]*16%mod<<" "<<n-(sz[s]-sz[t])<<"\n";
		}
	}
	printf("%lld\n",ans);
}
