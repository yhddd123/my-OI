...
// URL: https://atcoder.jp/contests/fps-24/tasks/fps_24_k
// Memory Limit: 1024 MB
// Time Limit: 2000 ms
// Written by yhm.
// Start codeing:2025-11-13 21:16:30
// 
...
void cdq(int l,int r){
	...
	vector<int> gg(r-l+1);
	for(int i=0;i<=r-l;i++)gg[i]=fac[i];
	ff=poly::mul(ff,gg);
	for(int i=mid+1;i<=r;i++)(f[i]+=ff[i-l])%=mod;
	cdq(mid+1,r);
}
void work(){
	n=read();init(n);
