...
void work(){
	...
	for(int j=0;j<=n;j++)dp[0][j]=inf;dp[0][0]=0;
	for(int i=1;i<=n;i++)q[i].id=i;
	for(int i=1;i<=m;i++){
		for(int j=0;j<=n;j++)sum[j]+=max(0,a[i]-w[j])*t;
		for(int j=0;j<=n;j++)dp[i][j]=inf;
		for(int j=0;j<=a[i];j++)vmn[j]=inf;
		for(int k=0;k<=n;k++)vmn[max(0,a[i]-w[k])]=min(vmn[max(0,a[i]-w[k])],dp[i-1][k]);
		for(int j=1;j<=n;j++){
		...
		for(int j=1;j<=n;j++)if(i-d[j]+1>0)dp[i][0]=min(dp[i][0],val[i-d[j]+1][j]+sum[j]);
		for(int j=1;j<=n;j++)dp[i][j]=min(dp[i][j],q[j].que(i-d[j]+2)+sum[j]);
		// cout<<i<<"\n";
		// for(int j=0;j<=n;j++)cout<<(dp[i][j]==inf?-1:dp[i][j])<<" ";cout<<"\n";
	}
	ll ans=inf;for(int j=0;j<=n;j++)ans=min(ans,dp[m][j]);
	printf("%lld\n",ans);
}
