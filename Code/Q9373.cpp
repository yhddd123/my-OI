...
int id[maxn],idx,rnk[maxn];
void downid(int u,int d){
	if(!d){
		if(!id[u])rnk[id[u]=++idx]=u;
		return ;
	}
...
void build(int nd,int l,int r){
	tag[nd]=0;
    if(l==r){mx[nd]=a[rnk[l]];return ;}
    build(ls,l,mid),build(rs,mid+1,r);
	mx[nd]=max(mx[ls],mx[rs]);
}
void upd(int nd,ll w){mx[nd]+=w,tag[nd]+=w;}
...
void work(){
	...
	for(int i=1;i<=n;i++){
		sub[i].clear();
		for(int j=0;j<=k;j++)kson[i][j].clear(),bro[i][j].clear();
	}
	dfs(1);
	lca_init();
	dfsid(1);
	...
	build(1,1,n);
	while(q--){
		int op=read(),u,k,w;ll ans=-inf;
		if(op==1){
			u=read(),k=read(),w=read();
			vector<pii> a=kson[u][k];
			for(int j=1,x=u;j<=k&&x;x=fa[x],j++)merge(a,bro[x][k-j]);
			reinit(a);
			for(auto[l,r]:a){
				updata(1,1,n,l,r,w);
				ans=max(ans,quemx(1,1,n,l,r));
			}
			if(ans==-inf)puts("GG");
			else printf("%lld\n",ans);
		}
		if(op==2){
			u=read(),k=read(),w=read();
			vector<pii> a;
			for(int j=0;j<=k;j++)merge(a,kson[u][j]);
			for(int j=1,x=u;j<=k&&x;x=fa[x],j++){
				for(int jj=0;jj<=k-j;jj++)merge(a,bro[x][jj]);
			}
			reinit(a);
			for(auto[l,r]:a){
				updata(1,1,n,l,r,w);
