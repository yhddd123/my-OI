...
// URL: https://atcoder.jp/contests/fps-24/tasks/fps_24_h
// Memory Limit: 1024 MB
// Time Limit: 2000 ms
// Written by yhm.
// Start codeing:2025-11-13 19:38:26
// 
...
int n,m,ans;
void work(){
	n=read();m=read();init(2*max(n,m));
	vector<int> f(m+1),g(m);
	for(int i=1;i<=m;i++)f[i]=C(n+i,n)*inv[i-1]%mod;
	for(int j=0;j<m;j++)g[j]=C(j+n-1,n-1)*fac[m-j-1]%mod;
	f=poly::mul(f,g);
	for(int i=1;i<=m;i++)(ans+=f[i]*inv[m-i])%=mod;
	(ans+=C(m+n-1,n-1))%=mod;
	printf("%lld\n",ans);
}
