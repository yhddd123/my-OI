...
void cdq(int l,int r){
	if(r-l+1<=1){
		for(int i=l;i<=r;i++){
			for(int j=l;j<i;j++)inc(f[i],C(i+1,j)*f[j]%mod);
			inc(f[i]=mod-f[i],ksm(m+1,i+1));
			f[i]=f[i]*inv[i+1]%mod*fac[i]%mod;
		}
		return ;
	}
	int mid=l+r>>1;
	...
	for(int i=l;i<=mid;i++)ff[i-l]=f[i]*inv[i]%mod;
...
void work(){
	...
	for(int i=0;i<=n;i++)ff[i]=inv[n-i]*ksm(m,n-i)%mod*f[i]%mod;
	for(int i=0;i<=n;i++)gg[n-i]=((i&1)?mod-1:1)*inv[i]%mod;
	ff=poly::mul(ff,gg);
	for(int j=1;j<n;j++){
		(ans+=C(n,j)*min(j,n-j)%mod*ff[n+j]%mod*fac[n-j])%=mod;
	}
	printf("%lld\n",ans);
}
