...
int dep[maxn],fa[maxn],siz[maxn],son[maxn],wf[maxn];
void dfs(int u){
    dep[u]=dep[fa[u]]+wf[u];siz[u]=1;
    for(int i=head[u];i;i=e[i].nxt){
        int v=e[i].to;if(v==fa[u])continue;
        fa[v]=u,wf[v]=e[i].w;dfs(v);siz[u]+=siz[v];
...
#define ls nd<<1
#define rs nd<<1|1
int tree[maxn<<2],mx[maxn<<2],tag[maxn<<2],val[maxn<<2];
void build(int nd,int l,int r){
    if(l==r){val[nd]=wf[rnk[l]];return ;}
    build(ls,l,mid),build(rs,mid+1,r);
    val[nd]=val[ls]+val[rs];
}
void updt(int nd,int w){tree[nd]+=w*val[nd],mx[nd]+=w,tag[nd]+=w;}
...
void work(){
    ...
    build(1,1,n);
    while(q--){
        int u=read(),w=read();num+=w;sum+=w*dep[u];
        upd(u,w);
        int p=query(1,1,n);
