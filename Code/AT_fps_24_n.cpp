...
// URL: https://atcoder.jp/contests/fps-24/tasks/fps_24_n
// Memory Limit: 1024 MB
// Time Limit: 2000 ms
// Written by yhm.
// Start codeing:2025-11-13 21:34:02
// 
...
void work(){
	...
	for(int i=1;i<=n;i++){
		int x=read();
		for(int p=1;p*i*(x+1)<=n;p++)(f[p*i*(x+1)]+=mod-ni[p])%=mod;
		for(int p=1;p*i<=n;p++)(f[p*i]+=ni[p])%=mod;
	}
	f=poly::exp(f);
	printf("%lld\n",f[n]);
}
