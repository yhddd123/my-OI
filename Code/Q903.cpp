...
void work(){
	...
	for(int i=1;i<=n;i++)if(g[i]+sum[i]==f[1])vis[i]=1;
	for(int i=1;i<=n;i++)if(vis[i])ff[i]=i,sz[i]=1;
	for(int u=1;u<=n;u++)if(vis[u]&&vis[fa[u]]){
		int x=fd(u),y=fd(fa[u]);
		ff[x]=y,sz[y]+=sz[x];
	}
	for(int i=1;i<=n;i++)if(fd(i)==i)ans+=sz[i]*sz[i];
	printf("%lld\n",ans);
}
