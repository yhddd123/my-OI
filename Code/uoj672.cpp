...
long long solve(int n, int x, int y, int q, vector<int> P, int L);
...
#define rs nd<<1|1
int tree[maxn<<2];
void upd(int nd,int w){tree[nd]=w;}
void down(int nd){upd(ls,tree[nd]),upd(rs,tree[nd]),tree[nd]=0;}
...
ll solve(int n, int x,int y, int q, vector<int> P, int L){
    vector<int> a(q+3);
    for(int i=1;i<=q;i++)a[i]=P[i-1];
    for(int i=q;i;i--)a[i+2]=a[i];a[1]=x,a[2]=y;q+=2;
    if(L==60000){
        vector<ll> pre(q+1),suf(q+1);
        for(int i=3;i<=q;i++)pre[i]=pre[i-1]+distance(a[i-2],a[i]);
        suf[q]=0;for(int i=q-1;i>=2;i--)suf[i]=suf[i+1]+distance(a[i],a[i+1]);
        ll res=pre[q];
        for(int i=2;i<q;i++)res=max(res,pre[i]+suf[i]);
        return res;
    }
    vector<ll> dp(2*n+1);
    for(int i=1;i<=n;i++)dp[i]=-inf;dp[x]=0;
    sgt::updata(1,1,2*n,1,2*n,x);
    unordered_map<ll,ll> mp;
    auto ask=[&](int u,int v){
        if(u>v)swap(u,v);
        if(mp[1ll*u*n+v])return mp[1ll*u*n+v];
        return mp[1ll*u*n+v]=distance(u,v);
    };
    auto calc=[&](int u,int v){
        if(v<u||v>u+n)return -inf;
        return dp[u]+ask(u,(v-1)%n+1);
    };
    ll add=0;
    for(int i=3;i<=q;i++){
    	ll dd=distance(a[i],a[i-1]);add+=dd;
        int pos=sgt::query(1,1,2*n,a[i]);
        if(pos!=a[i])dp[a[i-1]]=max(dp[a[i-1]],calc(pos,a[i])-dd);
        pos=sgt::query(1,1,2*n,a[i]+n);
        if(pos!=a[i])dp[a[i-1]]=max(dp[a[i-1]],calc(pos,a[i]+n)-dd);
        int l=1,r=2*n,pl=2*n,pr=1;
        while(l<=r){
            int p=sgt::query(1,1,2*n,mid);
            if(p<=a[i-1])pr=mid,l=mid+1;
            else r=mid-1;
        }
        l=1,r=2*n;
        while(l<=r){
            int p=sgt::query(1,1,2*n,mid);
            if(p>=a[i-1])pl=mid,r=mid-1;
            else l=mid+1;
        }
        // cout<<i<<" "<<dp[a[i-1]]<<" "<<pl<<" "<<pr<<"\n";
        int nl=pl,nr=pr;
        l=pl+1,r=2*n;
        while(l<=r){
            int p=sgt::query(1,1,2*n,mid);
            if(calc(p,mid)<calc(a[i-1],mid))nr=mid,l=mid+1;
            else r=mid-1;
        }
        l=1,r=pr-1;
        while(l<=r){
            int p=sgt::query(1,1,2*n,mid);
            if(calc(p,mid)<calc(a[i-1],mid))nl=mid,r=mid-1;
            else l=mid+1;
        }
        // cout<<nl<<" "<<nr<<"\n";
        sgt::updata(1,1,2*n,nl,nr,a[i-1]);
    }
    ll res=0;for(int i=1;i<=n;i++)res=max(res,dp[i]);res+=add;
    return res;
}
