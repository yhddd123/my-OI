...
namespace poly{
    ...
    int wg[20],wgg[20];
    void init(){
    	for(int i=0;i<20;i++)wg[i]=ksm(gen,(mod-1)/(1<<i));
    	for(int i=0;i<20;i++)wgg[i]=ksm(invg,(mod-1)/(1<<i));
    }
    void ntt(vector<int> &a,int fl){
        int n=a.size();
        for(int i=0;i<n;i++)if(i<to[i])swap(a[i],a[to[i]]);
        for(int l=2;l<=n;l<<=1){
            int bas=(fl==1?wg[__lg(l)]:wgg[__lg(l)]),k=l>>1;
            for(int i=0;i<n;i+=l){
                int mul=1;
                for(int j=i;j<i+k;j++){
...
void work(){
    ...
    f.resize(n+1);
    for(int i=0;i<=n;i++)f[i]=inv[i]*ksm(ksm(2,i*(i-1)/2))%mod;
    f=poly::mul(f,f);
    f[0]=1;for(int i=1;i<=n;i++)f[i]=fac[i]*ksm(2,i*(i-1)/2)%mod*f[i]%mod;
    for(int i=1;i<=n;i++)f[i]=inv[i]*f[i]%mod;
    f=poly::ln(f);
    // for(int i=1;i<=n;i++)f[i]=fac[i]*f[i]%mod;
    for(int i=1;i<=n;i++)f[i]=f[i]*(mod+1)/2%mod;
    // for(int i=1;i<=n;i++)f[i]=inv[i]*f[i]%mod;
    f=poly::exp(f);
    for(int i=1;i<=n;i++)f[i]=fac[i]*f[i]%mod;
